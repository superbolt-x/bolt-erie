...
WITH office_data as
    ...
    SELECT  
        ...
        CASE WHEN utm_campaign::varchar ~* 'Adv\\+' THEN TRIM(REPLACE(REPLACE(REPLACE(utm_campaign,'%28','('),'%29',')'),'%3A',':'))
            WHEN utm_campaign::varchar !~* 'Adv\\+' THEN TRIM(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(utm_campaign,'%28','('),'%29',')'),'%3A',':'),'+',' '),'%2B','+'))
...
SELECT 
    ...
    CASE WHEN utm_campaign_adj ~* 'Soc - Meta - Roofing - Prospecting - National - Adv All Areas 0000 - Lead - CBO (Lifetime)' THEN 'Soc - Meta - Roofing - Prospecting - National - Adv All Areas 0000 - Lead - CBO (Lifetime) Campaign' 
        ...
        WHEN utm_campaign_adj ~* 'Meta' AND utm_campaign_adj ~* 'Roofing' AND utm_campaign_adj ~* 'Retargeting' AND utm_campaign_adj ~* 'National' AND utm_campaign_adj ~* 'Old Structure' AND utm_campaign_adj ~* 'CBO' THEN 'Soc - Meta - Roofing - Retargeting - National - Old Structure All Areas Warm - Lead - CBO (Lifetime) Cost Cap'
